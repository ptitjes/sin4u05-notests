...
public class FXUtil {
    ...
    public interface AutoCompleteComparator<T> {
        boolean matches(String typedText, T objectToCompare, boolean exact);
    }
    ...
    public static <T> void autoCompleteComboBoxPlus(ComboBox<T> comboBox, AutoCompleteComparator<T> comparatorMethod) {
        ...
        comboBox.getSelectionModel().selectedIndexProperty().addListener((observable, oldValue, newValue) -> {
            for (T aData : data) {
                if (aData != null && comboBox.getEditor().getText() != null &&
                        comparatorMethod.matches(comboBox.getEditor().getText(), aData, true)) {
                    comboBox.setItems(data);
                    comboBox.getSelectionModel().select(aData);
                    return;
                }
            }
        });
        ...
        comboBox.addEventHandler(KeyEvent.KEY_RELEASED, new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                ...
                for (T aData : data) {
                    if (aData != null && comboBox.getEditor().getText() != null &&
                            comparatorMethod.matches(comboBox.getEditor().getText(), aData, false)) {
                        list.add(aData);
                    }
                }
                String t = "";
